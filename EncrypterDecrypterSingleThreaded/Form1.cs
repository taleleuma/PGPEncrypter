...
-----END PGP PRIVATE KEY BLOCK-----";
        ...
        private void DirSearchAndDecrypt(string sDir)
        {
            ...
            {
                WriteToLogFile(string.Format("Dir:{0} Error:{1}", sDir, ex.Message));
            }
        }
        ...
        private bool Decrypt(string sourceFile)
        {
            ...
            try
            {
                ...
                {
                    WriteToLogFile(string.Format("Sourcefile:{0} Error:{1}",sourceFile ,ex.Message));
                    throw;
                }
                finally
                {
         ...
         private void DirSearchAndEncrypt(string sDir)
        {
            ...
            {
                WriteToLogFile(string.Format("RootDir:{0} Error:{1}", sDir, ex.Message));
                RefreshMessage(false);
        ...
        private bool Encrypt(string fileNamePath)
        {
            ...
            try
            {
                ...
                {
                    WriteToLogFile(string.Format("Sourcefile:{0} Error:{1}", fileNamePath, ex.Message));
                    return false;
                }
                finally
                {
        ...
        private TElPGPPublicKey GetPublicKey(string KeyString)
        {
            ...
            {
                WriteToLogFile(string.Format("GetPublicKey Error:{0}", ex.Message));
                throw ex;
            }
            finally
            {
        ...
        private TElPGPSecretKey GetPrivateKey(string KeyString)
        {
            ...
            {
                WriteToLogFile(string.Format("GetPrivateKey Error:{0}", ex.Message));
                throw ex;
            }
            finally
            {
        ...
        private void btnEncryptRootPlusSubFolders_Click(object sender, EventArgs e)
        {
            ...
            this.folderBrowserDialog1.SelectedPath = Directory.Exists(@"C:\delete\1") ? @"C:\delete\1" : @"C:\";
        ...
        private void WriteToLogFile(string MsgString)
        {
            try
            {
                string lsMessage = String.Format("{0:MM/dd/yyyy HH:mm:ss} {1}", DateTime.Now, MsgString);
                TextWriter ltwErrorLog = null;

                try
                {
                    ltwErrorLog = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "EncrypterDecrypter.log"), true);

                    ltwErrorLog.WriteLine(lsMessage);
